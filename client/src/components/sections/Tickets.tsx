          ...
          {ticketBatches.map((batch, index) => {
            ...
            return (
              ...
              >
                {/* Batch Header */}
                <motion.button
                  // AQUI OCORRE O BLOQUEIO: Se estiver trancado, a função onClick não faz nada
                  onClick={() => {
                    if (!isLocked) {
                      setExpandedBatch(expandedBatch === index ? null : index);
                    }
                  }}
                  className={`w-full text-left p-6 rounded-lg shadow-md transition-all duration-300 flex items-center justify-between group focus-visible:outline-2 focus-visible:outline-[#5D2126] focus-visible:outline-offset-2 
                    ${isLocked ? 'cursor-default' : 'hover:shadow-lg cursor-pointer'} 
                  ...
                  disabled={isLocked} // Desabilita semanticamente o botão HTML
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      {/* Badge Em Breve */}
                      {batch.disabled && !batch.soldOut && (
                        <span className="px-3 py-1 bg-[#BC989A]/80 text-white text-xs font-bold rounded-full uppercase">EM BREVE</span>
                      )}
                    </div>
                  ...
                  {/* Seta: Só aparece se NÃO estiver trancado (nem esgotado, nem desabilitado) */}
                  {!isLocked && (
                    <motion.div
